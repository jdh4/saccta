...
df["gpu-hours"] = df["gpu-seconds"] / 3600.0
df.info()

if 0:
    sp = df[["account", "netid"]].drop_duplicates().copy()
    sp["sponsor-dict"] = sp.netid.apply(lambda netid: sponsor_per_cluster(netid, verbose=True)["della"])
...
#### ACCOUNT ####
if 1:
  ac = df.groupby("account").agg({"gpu-hours":np.sum, "netid":lambda series: len(set(series)), "account":np.size}).rename(columns={"account":"Jobs", "netid":"Number of Users"})
  ...
  ac.index += 1
  print(ac)
  caption = (f"GPU utilization by Slurm account on Della GPU from May 1, 2021 to January 31, 2023.", f"Della GPU -- Utilization by Slurm Account")
  ...
  fname = "dellagpu_account_feb1_2020_jan31_2023.tex"
  ac.to_latex(fname, index=True, caption=caption, column_format="rrrr", label=f"dellagpu_account_feb1_2020_jan31_2023", longtable=False)
  pad_multicolumn(fname, ["Number of Users", "GPU-hours"])

#import sys; sys.exit()
### SPONSOR ###
s = df[["netid", "gpu-hours"]].copy()
s = s.merge(sp, on="netid", how="left")
s = s.groupby("sponsor-dict").agg({"gpu-hours":np.sum, "netid":lambda series: len(set(series)), "account":lambda series: ",".join(sorted(set(series)))}).rename(columns={"sponsor-dict":"Jobs", "netid":"Number of Users"})
...
fname = "dellagpu_sponsor_feb1_2020_jan31_2023.tex"
caption = (f"GPU utilization by sponsor on Della GPU from May 1, 2021 to January 31, 2023.", f"Della GPU -- Utilization by Sponsor")
s.to_latex(fname, index=True, caption=caption, column_format="rrrrr", label=f"dellagpu_sponsor_feb1_2020_jan31_2023", longtable=True)
pad_multicolumn(fname, ["Number of Users", "GPU-hours"])


if 1:
  ### users ###
  users = df[["netid", "gpu-hours"]].copy()
  users = users.merge(sp, on="netid", how="left")
  users = users.groupby("netid").agg({"gpu-hours":np.sum, "sponsor-dict":min, "netid":np.size}).rename(columns={"netid":"Total Jobs", "sponsor-dict":"Sponsor", "gpu-hours":"GPU-hours"})
  ...
  users = users.sort_values(["SName", "GPU-hours"], ascending=[True, False]).reset_index(drop=True) # added on feb 8 2023
  users.index += 1
  ...
  users = users[["NetID", "GPU-hours", "Total Jobs", "Sponsor"]]
  print(users)
  print(users["GPU-hours"].sum())
  caption = (f"GPU utilization by user on Della GPU from May 1, 2021 to January 31, 2023.", f"Della GPU -- Utilization by User")
  users.to_latex("dellagpu_users_feb1_2020_jan31_2023.tex", index=True, caption=caption, column_format="rrrrr", label=f"dellagpu_users_feb1_2020_jan31_2023", longtable=True)
