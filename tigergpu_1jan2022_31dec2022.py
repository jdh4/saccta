...
# sacct -M tiger2 -a -X -P -S 2022-01-01T00:00:00 -E 2022-12-31T23:59:59 -o user,account,elapsedraw,alloctres --partition=gpu > raw.csv 
df = pd.read_csv("raw.csv", sep="|")
df = df.rename(columns={"User":"netid", "Account":"account", "ElapsedRaw":"elapsedraw", "AllocTRES":"alloctres"})
df.info()
...
print("gpu-seconds=", df["gpu-seconds"].sum())
#### ACCOUNT ####
if 1:
  ac = df.groupby("account").agg({"gpu-seconds":np.sum, "netid":lambda series: len(set(series)), "account":np.size}).rename(columns={"account":"Jobs", "netid":"Number of Users"})
  ac = ac.reset_index(drop=False)
  ac["gpu-hours"] = ac["gpu-seconds"] / 3600.0
  print("gpu-seconds=", ac["gpu-seconds"].sum())
  ac = ac[["account", "gpu-hours", "Number of Users", "Jobs"]].sort_values("gpu-hours", ascending=False)
  ac = ac.rename(columns={"account":"Account", "gpu-hours":"GPU-hours"})
  ac["Slurm Account"] = ac.Account.apply(lambda x: x.upper())
  ac = ac[["Slurm Account", "GPU-hours", "Number of Users"]].reset_index(drop=True)
  ac.index += 1
  print(ac)
  calc = "Account"
  caption = (f"GPU Utilization by Slurm {calc} on TigerGPU from January 1, 2022 to December 31, 2022.", f"TigerGPU -- Utilization by Slurm {calc}")
  print("GPU-hours=", ac["GPU-hours"].sum())
  ac = add_proportion_in_parenthesis(ac, "Number of Users", replace=True)
  ac = add_proportion_in_parenthesis(ac, "GPU-hours", replace=True)
  fname = f"tigergpu_{calc.lower()}_jan1_2022_dec31_2022.tex"
  ac.to_latex(fname, index=True, caption=caption, column_format="rrrr", label=f"tigergpu_{calc.lower()}_jan1_2022_dec31_2022", longtable=False)
  pad_multicolumn(fname, ["Number of Users", "GPU-hours"])

### SPONSOR ###
s = df[["netid", "gpu-seconds", "account"]].copy()
s = s.merge(sp, on="netid", how="left")
s = s.groupby("sponsor-dict").agg({"gpu-seconds":np.sum, "netid":lambda series: len(set(series)), "account":lambda series: ",".join(sorted(set(series)))}).rename(columns={"sponsor-dict":"Jobs", "netid":"Number of Users"})
print("gpu-seconds=", s["gpu-seconds"].sum())
s["gpu-hours"] = s["gpu-seconds"] / 3600.0
s["Slurm Accounts"] = s.account.apply(lambda x: x.upper())
s = s.reset_index(drop=False)
s = s.rename(columns={"sponsor-dict":"Sponsor", "gpu-hours":"GPU-hours"})
s = s[["Sponsor", "Slurm Accounts", "GPU-hours", "Number of Users"]].sort_values("GPU-hours", ascending=False)
...
s = s[["Sponsor", "Slurm Accounts", "GPU-hours", "Number of Users"]]
print(s["GPU-hours"].sum())
s = add_proportion_in_parenthesis(s, "Number of Users", replace=True)
s = add_proportion_in_parenthesis(s, "GPU-hours", replace=True)
calc = "Sponsor"
caption = (f"GPU Utilization by {calc} on TigerGPU from January 1, 2022 to December 31, 2022.", f"TigerGPU -- Utilization by {calc}")
fname = f"tigergpu_{calc.lower()}_jan1_2022_dec31_2022.tex"
s.to_latex(fname, index=True, caption=caption, column_format="rrrrr", label=f"tigergpu_{calc.lower()}_jan1_2022_dec31_2022", longtable=True)
pad_multicolumn(fname, ["Number of Users", "GPU-hours"])


if 1:
  ### users ###
  users = df[["netid", "gpu-seconds", "account"]].copy()
  users = users.merge(sp, on="netid", how="left")
  users = users.groupby("netid").agg({"gpu-seconds":np.sum, "sponsor-dict":min, "netid":np.size, "account":lambda series: ",".join(sorted(set(series)))}).rename(columns={"netid":"Total Jobs", "sponsor-dict":"Sponsor", "account":"Account"})
  users["gpu-hours"] = users["gpu-seconds"] / 3600.0
  users = users.rename(columns={"gpu-hours":"GPU-hours"})
  ...
  users = users.sort_values(["ToSort", "GPU-hours"], ascending=[True, False]).reset_index(drop=True) # added on feb 8 2023
  ...
  print("gpu-seconds=", users["gpu-seconds"].sum())
  users["GPU-hours"] = users["GPU-hours"].apply(round)
  users = users[["NetID", "Account", "GPU-hours", "Total Jobs", "Sponsor"]]
  #print(users)
  print(users["GPU-hours"].sum())
  calc = "User"
  caption = (f"GPU Utilization by {calc} on TigerGPU from January 1, 2022 to December 31, 2022.", f"TigerGPU -- Utilization by {calc}")
  fname = f"tigergpu_{calc.lower()}_jan1_2022_dec31_2022.tex"
  users.to_latex(fname, index=True, caption=caption, column_format="rrrrrr", label=f"tigergpu_{calc.lower()}_jan1_2022_dec31_2022", longtable=True)
